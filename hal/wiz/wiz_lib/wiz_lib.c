...
//int wiz_sound_stop = 1;
unsigned short  wiz_sound_buffer[4+ ( (44100*2) *8) ];
//unsigned short  wiz_sound_buffer[4+ ( (640*2) *8) ];
volatile unsigned int samplesTotales = 0;
volatile short *pOutput[8]; 
volatile int CurrentSoundBank=0;
static volatile unsigned long  wiz_sound_pausei, wiz_sound_exit;
...
#if 0
static void *wiz_sound_play(void *blah)
{
#ifdef DEBUG
	printf ("Se lanza el hilo de sonido\n");
#endif

	int flip1 = 0, flip2 = wiz_sound_buffer[1];
	wiz_sound_exit = 0;
	//struct timespec ts; ts.tv_sec=0, ts.tv_nsec=(gp2x_sound_buffer[3]<<16)|gp2x_sound_buffer[2];

	while (!wiz_sound_exit)      //nanosleep(&ts, NULL);
	{
		if (!wiz_sound_pausei)
		{
			soundFrame(blah, (void *)(&wiz_sound_buffer[4+flip1]), wiz_sound_buffer[0]);
			write(wiz_dev[1],     (void *)(&wiz_sound_buffer[4+flip2]), wiz_sound_buffer[1]);
			ioctl(wiz_dev[1], SOUND_PCM_SYNC, 0);

			flip1 += wiz_sound_buffer[1];

			if (flip1 == wiz_sound_buffer[1] * 8) flip1 = 0;

			flip2 += wiz_sound_buffer[1];

			if (flip2 == wiz_sound_buffer[1] * 8) flip2 = 0;
		}
	}
...
#ifdef DEBUG
	printf ("Se sale del hilo de sonido\n");
#endif	
}
#endif
...
static void *wiz_sound_play(void)
{
	...
	printf ("Addr inicial: %u\n", (void *)pOutput);

	while (!wiz_sound_exit)    
	{
		if (!wiz_sound_pausei)
		{
			CurrentSoundBank++;
			...
			escritos = write(wiz_dev[1],     (void *)pOutput[CurrentSoundBank], wiz_sound_buffer[1]);
			if (escritos != wiz_sound_buffer[1])
			{
				perror("Wiz_sound_play");
				printf ("Bank: %d - Addr: %u\n", CurrentSoundBank, pOutput[CurrentSoundBank]);
			}
			ioctl(wiz_dev[1], SOUND_PCM_SYNC, 0);
			samplesTotales++;
			//usleep(1000);
...
void wiz_sound_init (int rate, int bits, int stereo, int Hz)
{
	...
	if (ioctl(wiz_dev[1], SNDCTL_DSP_SETFRAGMENT, &frag) == -1)
		perror("DSP Setfragment");
	if (ioctl(wiz_dev[1], SNDCTL_DSP_GETOSPACE, &abi) == -1)
		perror("DSP Getospace");
	...
	*/
	...
	if (ioctl (wiz_dev[1], SNDCTL_DSP_SPEED,  &rate) == -1)
		perror("DSP Speed");
	/*
	while (ioctl (wiz_dev[1], SNDCTL_DSP_SPEED,  &rate) == -1)
	{
		//perror("DSP Speed");
		rate++;
	}
	*/
	if (ioctl (wiz_dev[1], SNDCTL_DSP_SETFMT, &bits) == -1)
		perror("DSP Bits");
	...
	/*
	printf ("Sound pOutput[0]: %u\n", pOutput[0]);
	printf ("Sound pOutput[1]: %u\n", pOutput[1]);
	printf ("Sound pOutput[2]: %u\n", pOutput[2]);
	*/
